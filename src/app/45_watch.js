...
(function () {
  ...
  function persist() {
    try { localStorage.setItem(KEY, JSON.stringify(rows.map(function (r) { return { expr: r.expr }; }))); } catch (e) {}
  }
  ...
  function poll(row) {
    ...
    IDE.bridge.run("return (" + row.expr + "\n)").then(function (r) {
      ...
      if (r.error) setStatus(row, r.error, "err");
      else if (r.timedOut) setStatus(row, "timed out", "err");
      else if (r.ok === false) setStatus(row, "error: " + r.value, "err");
      else setStatus(row, (r.value === "" || r.value == null) ? "nil" : r.value, "ok");
    });
  }
